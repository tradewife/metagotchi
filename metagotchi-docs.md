...
## Skills in Metagotchi

Skills still exist, but their role is sharply limited.

### What skills are for

In Metagotchi, skills are reference modules the harness reads and injects. They are not the execution path. Appropriate skill content includes:
...
Metagotchi implements a strict planner-generator-evaluator separation using the migrated harness structure.
...
| Generator | Model via prompt package | Produce candidate solutions under the contract | Always-on [file:1][file:22] |
...
Metagotchi uses classifier-derived token budgets for the initial solve prompt. The migrated rule is:
...
**The gotchas block is never truncated.** When prompt size exceeds budget, Metagotchi drops retrieved solutions first, then compresses lower-priority context. Gotchas and risk warnings stay.[file:22]
...
The gotchas registry is Metagotchi’s highest-priority reusable memory. This aligns with both the original harness spec and Anthropic’s observed best practice that gotchas are usually the highest-signal part of any skill or reusable instruction set.[file:1][file:6]
...
On non-AC results, Metagotchi should analyze the failure, increment matching gotchas where appropriate, and append a new gotcha when a materially new failure pattern is discovered. Gotchas are append-friendly institutional memory; they should grow conservatively but continuously.[file:1]

## Verification policy

Metagotchi must verify before it believes. The verifier is not cosmetic — it is the skeptical counterweight to generation.[file:1][file:2]
...
This is conceptually similar to what Anthropic calls product verification skills: deterministic or script-backed validation that improves output reliability. In Metagotchi, however, that verification is baked into the runtime rather than left as a user-invoked skill.[file:6][file:22]

## Logging and memory

Metagotchi must log **raw traces, not summaries**. This rule comes from both the original harness spec and Meta-Harness: compressed summaries often erase the causal evidence needed to diagnose why a harness or candidate failed.[file:1][file:3]
...
Metagotchi should treat the local filesystem as the real long-term memory substrate.[file:2][file:3]
...
Meta-Harness argues that the proposer should reason directly over prior code and execution traces, not over thin summaries generated by an outer controller. Metagotchi should preserve that philosophy.[file:3]

## AGENTS.md policy

`AGENTS.md` is the universal, cross-agent entry point for persistent Metagotchi behavior. It should contain only rules that truly must apply every session.[file:22]

### What belongs in AGENTS.md

- Metagotchi identity and mission
...
Treat Metagotchi as a deterministic competitive programming runtime. Do not reinterpret it as a menu of optional skills.
